...
useEffect(() => {
    if (editando && producto.data) {
      form.reset({
        ...
        imagenes: producto.data.imagenes.length > 0 ? producto.data.imagenes : ['placeholder.jpg'],
        tiposEntrega: producto.data.tiposEntrega.length > 0 ? producto.data.tiposEntrega : ['recoger'],
      });
    }
